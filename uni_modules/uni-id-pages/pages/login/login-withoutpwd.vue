	...
	<view class="uni-content">
		...
		<template v-if="['apple','weixin', 'weixinMobile'].includes(type)">
			...
			<view class="quickLogin">
				...
				<view class="quick-login-actions">
					<button type="primary" open-type="getPhoneNumber" @getphonenumber="quickLogin"
						class="phone-login-button">手机号快捷登录</button>
					<button class="no-login-button" @click="noLogin">暂不登录</button>
				</view>
		...
		<!-- 固定定位的快捷登录按钮 -->
		<uni-id-pages-fab-login style="display: none;" ref="uniFabLogin"></uni-id-pages-fab-login>
	</view>
</template>
		...
		methods: {
			...
			quickLogin(e) {
				if (e.detail?.errMsg != "getPhoneNumber:ok") {
					console.log("UCenter, quickLogin, failed: ", e.detail?.errMsg)
					return;
				}
				let options = {}
				if (e.detail?.code) {
