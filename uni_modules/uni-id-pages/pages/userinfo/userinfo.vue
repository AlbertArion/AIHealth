...
<template>
	<view class="uni-content">
		...
		<uni-popup ref="dialog" type="dialog">
			<uni-popup-dialog mode="input" :value="userInfo.nickname" @close="closeNicknameDialog"
				@confirm="setNickname" title="设置昵称" placeholder="请输入要设置的昵称">
			</uni-popup-dialog>
		</uni-popup>
		...
		<template v-if="showLoginManage">
			<button class="login-button" v-if="userInfo._id" @click="logout">退出登录</button>
			<button class="login-button" v-else @click="login" style="font-size: 14px;">去登录</button>
		</template>
	</view>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			closeNicknameDialog() {
				console.log("UserInfo, nickname dialog close");
				store.setNicknameIng = false;
			},
			setNickname(nickname) {
				if (nickname) {
					...
					console.log("UserInfo, nickname dialog open");
					store.setNicknameIng = true;
