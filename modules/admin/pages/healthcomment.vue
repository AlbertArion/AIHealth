<template>
	<scroll-view class="comment-content" scroll-y="true" @scrolltolower="scrolltolower">
		<record-item v-if="hasReports" v-for="(record, index) in records" :from="'comment'" v-bind:record="record"
			:id="index" :key="index" />
		...
		methods: {
			...
			getQuery: async function() {
				...
				db.collection('health-records')
					...
					.get()
					.then((res) => {
						console.log("HealthComment, dbrecords: ", res);
						...
						console.log("Records, 获取电子病历失败, err = " + JSON.stringify(err));
					}).finally(() => {
						this.isInQuery = false;
