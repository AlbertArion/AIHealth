	...
	export default {
		...
		methods: {
			...
			async login_before(type, navigateBack = true, options = {}) {
				console.log("FabLogin, login_before, type = " + type + ", options: ", options);
				//提示空实现
				if (["qq",
						"xiaomi",
						"sinaweibo",
				...
				if (agreementsRef && type != 'univerify' && needAgreements && !this.agree) {
				...
				// #endif
				console.log("before login, showloading");
				uni.showLoading({
					title: '加载中...',
					mask: true
				});
				...
				uni.login({
					...
					success: async e => {
						...
						this.login(type == 'weixin' ? {
							code: e.code,
							phoneNumberCode: options.phoneNumberCode,
						} : e.authResult, type)
					},
					fail: async (err) => {
						console.log(err);
			...
			login(params, type) {
				...
				// uni.setStorageSync('userCode', params.code);
