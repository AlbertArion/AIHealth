...
function fetchPointColor(strokeColor, opts, index) {
	return opts.dataPointColor?.length > 0 ? opts.dataPointColor[index] : strokeColor;
}

function drawPointShape(points, color, shape, context, opts) {
	context.beginPath();
	const shapeType = opts.dataPointShapeType;
	const strokeColor = color || opts.color;
	if (shape === 'diamond') {
		context.setStrokeStyle(strokeColor);
		context.setFillStyle(shapeType == 'hollow' ? opts.background : strokeColor);
		...
		points.forEach(function(item, index) {
			const pointColor = fetchPointColor(strokeColor, opts, index);
			if (item !== null && pointColor != 'transparent') {
				context.closePath();
				context.fill();
				context.stroke();
				context.beginPath();
				// context.setLineWidth(2 * opts.pix);
				context.setStrokeStyle(pointColor);
				context.setFillStyle(shapeType == 'hollow' ? 'transparent' : strokeColor);
		...
		points.forEach(function(item, index) {
			...
			if (item !== null) {
				...
				context.arc(item.x, item.y, opts.pix, 0, 2 * Math.PI, false);
			}
		});
...
function drawPointText(points, series, config, context, opts) {
	...
	const strokeColor = series.color || opts.color;
	points.forEach(function(item, index) {
		const pointColor = fetchPointColor(strokeColor, opts, index);
		if (item !== null && pointColor != 'transparent') {
			context.beginPath();
...
function drawYAxisGrid(categories, opts, config, context) {
	...
	points.forEach(function(item, index) {
		...
		let strokeColor = yAxis.gridColor;
		if (opts.yAxis.drawGrid) {
			strokeColor = yAxis.drawGrid(index) ? yAxis.gridColor : 'transparent';
		} else {
			strokeColor = opts.yAxis.gridColor;
		}
		context.setStrokeStyle(strokeColor);
		context.beginPath();
	...
	// 	};
	// 	context.beginPath();
	// 	const gradient1 = context.createLinearGradient(0, 0, 0, eachSpacing * 1.5);
	// 	gradient1.addColorStop(0.00, 'rgba(222,111,107,1)');
	// 	gradient1.addColorStop(1.00, 'rgba(222,111,107,1)');
	// 	context.rect(sectionAxis.start.x, sectionAxis.start.y,
	// 		sectionAxis.size.width, eachSpacing * 1.5);
	...
	// 	context.setFillStyle('#FFFFFF');
	// 	context.fillText('高风险区', sectionAxis.start.x + TotalWidth * 3 / 7, sectionAxis.start.y + eachSpacing);
	// 	context.closePath();
	// 	context.stroke();

	// 	context.beginPath();
	// 	const gradient2 = context.createLinearGradient(0, 0, 0, eachSpacing * 2);
	// 	gradient2.addColorStop(0.00, 'rgba(241,201,106,1)');
	// 	gradient2.addColorStop(1.00, 'rgba(241,201,106,1)');
	// 	context.rect(sectionAxis.start.x, sectionAxis.start.y + eachSpacing * 1.5,
	// 		sectionAxis.size.width, eachSpacing * 2);
	...
	// 	context.setFontSize(10);
	// 	context.setFillStyle('#FFFFFF');
	// 	context.fillText('低风险区', sectionAxis.start.x + TotalWidth * 3 / 7, sectionAxis.start.y + eachSpacing * 2.7);
	// 	context.closePath();
	// 	context.stroke();

	// 	context.beginPath();
	// 	const gradient3 = context.createLinearGradient(0, 0, 0, eachSpacing * 3);
	// 	gradient3.addColorStop(0.00, 'transparent');
	// 	gradient3.addColorStop(1.00, 'transparent');
	// 	context.rect(sectionAxis.start.x, sectionAxis.start.y + eachSpacing * 3.5,
	// 		sectionAxis.size.width, eachSpacing * 3);
	// 	context.setFillStyle(gradient3);
	// 	context.fill();
	// 	context.closePath();
	// 	context.stroke();
	...
	// 	context.beginPath();
	// 	const gradient4 = context.createLinearGradient(0, 0, 0, eachSpacing * 2.5);
	// 	gradient4.addColorStop(0.00, 'rgba(241,201,106,1)');
	// 	gradient4.addColorStop(1.00, 'rgba(241,201,106,1)');
	// 	context.rect(sectionAxis.start.x, sectionAxis.start.y + eachSpacing * 6.5,
	// 		sectionAxis.size.width, eachSpacing * 2.5);
	// 	context.setFillStyle(gradient4);
	// 	context.fill();
	// 	context.closePath();
	// 	context.stroke();
	...
	// 	context.setFontSize(10);
	// 	context.setFillStyle('#FFFFFF');
	// 	context.fillText('低风险区', sectionAxis.start.x + TotalWidth * 3 / 7, sectionAxis.start.y + eachSpacing * 8.0);
	// 	context.closePath();
	// 	context.stroke();
	// }
	context.setLineDash([]);
	...
	if (opts.yAxis.data) {
		for (let i = 0; i < opts.yAxis.data.length; i++) {
			...
			if (yData.disabled !== true) {
				...
				// 计算Y轴坐标文字偏移量
				let yAxisOffset = 0;
				const yAxisDelta = yAxisFontSize / 2 - 3 * opts.pix;
				if (opts.yAxis.splitType == 'range' && rangesFormat.length > 1) {
					yAxisOffset = (points[1] - points[0]) / 2;
				}
				//画Y轴刻度及文案
				rangesFormat.forEach(function(item, index) {
					...
					if (yAxisWidth.position == 'left') {
						...
						context.fillText(String(item), tmpstrat, pos + yAxisDelta + yAxisOffset);
				...
				if (opts.yAxis.showTitle) {
					...
					if (yAxisWidth.position == 'left') {
						...
						context.fillText(title, tStartRight - measureText(title, titleFontSize, context) / 2 + (
							yData
							.titleOffsetX || 0), opts.area[0] - (10 - (yData.titleOffsetY || 0)) * opts.pix);
					} else if (yAxisWidth.position == 'center') {
						context.fillText(title, tStartCenter - measureText(title, titleFontSize, context) / 2 + (
							yData
							.titleOffsetX || 0), opts.area[0] - (10 - (yData.titleOffsetY || 0)) * opts.pix);
					}
					context.closePath();
...
function drawPieDataPoints(series, opts, config, context) {
	...
	var radius = Math.min((opts.width - opts.area[1] - opts.area[3]) / 2 - config.pieChartLinePadding - config
		.pieChartTextPadding - config._pieTextMaxLength_, (opts.height - opts.area[0] - opts.area[2]) / 2 -
		config
	...
	series.forEach(function(eachSeries, seriesIndex) {
		if (opts.tooltip) {
			if (opts.tooltip.index == seriesIndex) {
				...
				context.arc(centerPosition.x, centerPosition.y, eachSeries._radius_ + activeRadius,
					eachSeries
		...
		if (pieOption.linearType == 'custom') {
			...
			if (context.createCircularGradient) {
				grd = context.createCircularGradient(centerPosition.x, centerPosition.y, eachSeries
					._radius_)
			} else {
	...
	series.forEach(function(eachSeries, seriesIndex) {
		if (opts.tooltip) {
			if (opts.tooltip.index == seriesIndex) {
				...
				context.moveTo(centerPosition.x, centerPosition.y);
				context.arc(centerPosition.x, centerPosition.y, activeRadius + eachSeries._radius_,
					eachSeries
...
function drawArcbarDataPoints(series, opts, config, context) {
	...
	for (let i = 0; i < series.length; i++) {
		...
		if (arcbarOption.type == 'default') {
			context.arc(centerPosition.x, centerPosition.y, radius - (arcbarOption.width * opts.pix + arcbarOption
					.gap *
					opts.pix) * i, arcbarOption.startAngle * Math.PI, arcbarOption.endAngle * Math.PI,
				arcbarOption
				.direction == 'ccw');
		} else {
			context.arc(centerPosition.x, centerPosition.y, radius - (arcbarOption.width * opts.pix + arcbarOption
		...
		context.arc(centerPosition.x, centerPosition.y, radius - (arcbarOption.width * opts.pix + arcbarOption.gap *
				opts.pix) * i, arcbarOption.startAngle * Math.PI, eachSeries._proportion_ * Math.PI,
			arcbarOption
			.direction == 'ccw');
		context.stroke();
	}
...
function drawGaugeDataPoints(categories, series, opts, config, context) {
	...
	if (gaugeOption.type == 'progress') {
		...
		context.beginPath();
		let gradient = context.createLinearGradient(centerPosition.x, centerPosition.y - pieRadius, centerPosition
			.x,
		...
		let childAngle = totalAngle / gaugeOption.splitLine.splitNumber / gaugeOption.splitLine.childNumber;
		let startX = -radius - gaugeOption.width * 0.5 - gaugeOption.splitLine.fixRadius;
		let endX = -radius - gaugeOption.width * 0.5 - gaugeOption.splitLine.fixRadius + gaugeOption.splitLine
			.width;
...
function drawRadarDataPoints(series, opts, config, context) {
	...
	coordinateAngle.forEach(function(angle, index) {
		var pos = convertCoordinateOrigin(radius * Math.cos(angle), radius * Math.sin(angle),
			centerPosition);
		context.moveTo(centerPosition.x, centerPosition.y);
	...
	radarDataPoints.forEach(function(eachSeries, seriesIndex) {
		...
		if (radarOption.linearType == 'custom') {
			...
			grd.addColorStop(0, hexToRgb(radarOption.customColor[series[seriesIndex].linearIndex],
				radarOption
				.opacity))
			...
			eachSeries.data.forEach(function(item, index) {
				...
				if (Math.abs(item.position.x - centerPosition.x) < 2) {
					...
					if (item.position.y < centerPosition.y) {
						...
						context.setTextAlign('center');
						context.fillText(item.value, item.position.x, item.position.y + fontSize +
							2);
					}
				} else {
					//如果在左侧
					if (item.position.x < centerPosition.x) {
						context.setTextAlign('right');
						context.fillText(item.value, item.position.x - 4, item.position.y +
							fontSize /
							2 - 2);
					} else {
						context.setTextAlign('left');
...
function collisionNew(area, points, width, height) {
	...
	for (let i = 0; i < points.length; i++) {
		if (points[i].area) {
			if (area[3] < points[i].area[1] || area[0] > points[i].area[2] || area[1] > points[i].area[3] || area[
					2] <
				points[i].area[0]) {
				...
				while (true) {
					...
					if (isSpin) {
						...
						areav = [opts.width - (opts.width / 2 - opts.height / 2) - (-x + tHeight + 5 + opts.height /
								2) - 5, (opts.height / 2 - opts.width / 2) + (y - 5 - tWidth + opts.width / 2) -
							5,
							opts
						...
						x = normalInt(-opts.width / 2, opts.width / 2, 5) - tWidth / 2;
						y = normalInt(-opts.height / 2, opts.height / 2, 5) + tHeight / 2;
						area = [x - 5 + opts.width / 2, y - 5 - tHeight + opts.height / 2, x + tWidth + 5 + opts
							.width /
							2, y + 5 + opts.height / 2
						];
...
function drawFunnelDataPoints(series, opts, config, context) {
	...
	let radius = Math.min((opts.width - opts.area[1] - opts.area[3]) / 2 - activeWidth, (opts.height - opts.area[
			0] -
...
function drawFunnelCenterText(series, opts, context, eachSpacing, labelAlign, activeWidth, centerPosition) {
	for (let i = 0; i < series.length; i++) {
		...
		if (item.centerText) {
			...
			context.fillText(item.centerText, centerPosition.x - measureText(item.centerText, fontSize, context) /
				2,
				startY + fontSize / 2 - 2);
			context.closePath();
			context.stroke();
...
function drawCharts(type, opts, config, context) {
	...
	if (opts.categories && opts.categories.length && opts.type !== 'radar' && opts.type !== 'gauge' && opts.type !==
		...
		let _calCategoriesData = calCategoriesData(opts.categories, opts, config, opts.chartData.xAxisData
				.eachSpacing,
				context),
		...
		if (opts.type === 'line' || opts.type === 'area' || opts.type === 'scatter' || opts.type === 'bubble' ||
			...
			let _calCategoriesData = calCategoriesData(categories, opts, config, opts.chartData.xAxisData
					.eachSpacing,
					context),
				xAxisHeight = _calCategoriesData.xAxisHeight,
				angle = _calCategoriesData.angle;
		...
		case 'mount':
			this.animationInstance = new Animation({
				...
				onProcess: function onProcess(process) {
					...
					drawYAxisGrid(categories, opts, config, context);
					drawXAxis(categories, opts, config, context);
					var _drawMountDataPoints = drawMountDataPoints(series, opts, config, context,
							process),
		...
		case 'area':
			this.animationInstance = new Animation({
				...
				onProcess: function onProcess(process) {
					...
					drawYAxisGrid(categories, opts, config, context);
					drawXAxis(categories, opts, config, context);
					var _drawAreaDataPoints = drawAreaDataPoints(series, opts, config, context,
							process),
		...
		case 'radar':
			this.animationInstance = new Animation({
				...
				onProcess: function onProcess(process) {
					...
					opts.chartData.radarData = drawRadarDataPoints(series, opts, config, context,
						process);
					drawLegend(opts.series, opts, config, context, opts.chartData);
					drawToolTipBridge(opts, config, context, process);
					drawCanvas(opts, context);
...
uCharts.prototype.zoom = function() {
	...
	//当前屏幕中间点
	let centerPoint = Math.round(Math.abs(this.scrollOption.currentOffset) / this.opts.chartData.eachSpacing) +
		Math
...
uCharts.prototype.showToolTip = function(e) {
	...
	if (this.opts.type === 'line' || this.opts.type === 'area' || this.opts.type === 'column' || this.opts
		.type === 'scatter' || this.opts.type === 'bubble') {
	...
	if (this.opts.type === 'candle') {
		...
		if (index > -1) {
			...
			var seriesData = getSeriesDataItem(this.opts.series, index);
			if (seriesData.length !== 0) {
				var _getToolTipData = getCandleToolTipData(this.opts.series[0].data, seriesData, this.opts,
						index,
		...
		if (index > -1) {
			...
			if (seriesData.length !== 0) {
				var textList = seriesData.map((item) => {
					return {
						text: option.formatter ? option.formatter(item, this.opts.categories[index],
							index,
...
uCharts.prototype.scroll = function(e) {
	...
	if (touches && this.opts.enableScroll === true) {
		...
		var currentOffset = this.scrollOption.currentOffset;
		var validDistance = calValidDistance(this, currentOffset + _distance, this.opts.chartData, this.config,
			this
			.opts);
