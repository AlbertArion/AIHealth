<template>
	<view class="pay-content">
		...
		<view v-if="isPaid" class="pay-tips-container">
			...
			<text class="pay-result-content"> - 健康建议将于12小时以内完成后，在微信小程序「心脉AI」的消息提醒中通知您；</text>
			<text class="pay-result-content"> - 您也可以在「心脉AI」小程序内「报告解读」中查看。</text>
		</view>
		...
		<!-- 统一支付组件，注意：vue3下ref不可以等于组件名，因此这里ref="pay" 而不能是 ref="uniPay"
			- :adpid="adpid"可以增加广告位
			- return-url="/pages/pay/aipay?res=success"可以增加查看订单页
		-->
...
<script>
	...
	export default {
		...
		onLoad(options = {}) {
			...
			console.log("AiPay, onLoad, options = ", options);
			if (options.index) {
		...
		methods: {
			...
			onPayClick: function(provider) {
				if (uni.subscribed) {
					...
					uni.requestSubscribeMessage({
						...
						complete: (data) => {
							console.log("AiPay, subscribe success", data);
							this.createOrder(provider);
						}
					});
				}
			...
			createOrder(provider) {
				...
				this.$refs.pay.createOrder({
					...
					console.log("AiPay, createOrder, res = ", res);
				});
			},
			/**
			 * 生成支付独立二维码（只返回支付二维码）
			...
			onCreate(res) {
				console.log('AiPay, pay onCreate: ', res);
			...
			// 监听事件 - 支付成功
			onSuccess(res) {
				console.log('AiPay, pay onSuccess: ', res);
				if (res) {
					const obj = {
						"-1": "已关闭",
						"1": "已支付",
					...
					if (this.record) {
						uniCloud.database().collection('health-records').where({
							...
							console.log("AiPay, AI报告解读支付成功");
							this.updateOrder();
						}).catch((err) => {
							console.log("AiPay, AI报告解读支付失败: ", err);
						}).finally(() => {});
					} else {
						this.updateOrder();
					}
			...
			onCancel(res) {
				console.log('AiPay, pay onCancel: ', res);
				uni.showToast({
					title: "支付取消",
					icon: "none"
				});
			...
			navigateBack: function() {
				if (this.record) {
					uni.$emit('aiPaid', this.index);
				} else {
					uni.$emit('servicePaid', this.total_fee);
				}
				uni.navigateBack();
			}
		}
