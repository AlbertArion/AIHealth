<template>
	<view @appear="appear">
		<view v-if="state.error">
			<view class="box" v-if="networkType == 'none'">
				<image class="icon-image"
					src="https://mp-d4a734ea-9ae3-4448-99e4-ae65b22bbaf4.cdn.bspapp.com/cloudstorage/disconnection.png"
					mode="widthFix"></image>
		...
		<template v-else>
			...
			<!-- #endif -->
			<!-- #ifndef APP-NVUE -->
			<uni-load-more class="uni-load-more"
				:status="state.loading?'loading':(state.hasMore?'hasMore':'noMore')"></uni-load-more>
			<!-- #endif -->
		</template>
