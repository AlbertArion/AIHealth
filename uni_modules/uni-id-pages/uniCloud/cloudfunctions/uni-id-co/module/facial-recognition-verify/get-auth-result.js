...
const {
	encryptData,
	decryptData
} = require('../../common/sensitive-aes-cipher')
const {
	ERROR
...
module.exports = async function(params) {
	...
	const {
		uid
	} = this.authInfo // 从authInfo中取出uid属性
	const {
		certifyId
	} = params // 从params中取出certifyId属性
	...
	if (res.pictureUrl) {
		...
		if (pictureRes.status < 400) {
			const {
				...
				cloudPath: `upload/user/id-card/${uid}.b64`,
	...
	return {
		...
		realName: dataDesensitization(decryptData.call(this, log.real_name), {
			onlyLast: true
		}), // 对姓名进行脱敏处理
