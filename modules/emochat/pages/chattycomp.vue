		...
		<view class="uni-column" style="overflow-y: hidden;">
			...
			<view class="dialog-content" id="content">
				<scroll-view class="dialog-scroll-view" upper-threshold="100" id="scrollview" scroll-y="true"
					...
					<view v-if="shouldLoadMessages" class="scroll-header">
			...
			<view class="float-container" id="bottom-input" :style="`width: ${style.windowWidth}px;}`">
				<chatty-input ref="chatInput" @show-history="showHistory" @send-message="getInputMessage"
					@last-message="getLastMessage" @end-call="endCall" :color="color" :popupShow="popupShow"
	...
	export default {
		...
		methods: {
			...
			endDialog: function() {
				...
				const collName = 'chatai-dialog-list';
			...
			pushGuideMessage: function(messages = this.messages) {
				const welcome = [{
					...
					approve: false,
					disapprove: false,
					content: '欢迎使用Emohaa，我是您的Emohaa助手，您有什么问题需要咨询吗？'
				}];
			...
			scrollToBottom: function() {
				...
				this.calculateItemHeight(() => {
					that.scrollTop = that.style.itemHeight;
				});
				// #endif
			},
			...
			wxRobatRequest: function(question, id, type) {
				...
				uni.request({
					url: getApp().globalData.isDevTools() ? 'https://test.aicareme.cn/emo_chat' :
						'https://www.aicareme.cn/emo_chat',
					method: 'POST',
					data: requestData,
					success: (res) => {
						console.log("ChattyComp, emo_chat success", res);
						if (res.data.status == 200) {
							writingContent = res.data.data.content;
							console.log(writingContent, 'writingContent');
							this.$refs.chatInput.aiSpeaking(writingContent);
							this.addStreamMessage('', id);
							if (!writingTimer) {
					...
					fail: (err) => {
						...
						this.queueCount++;
						console.log("ChattyComp, emo_chat failed: ", err);
					},
					complete: () => {
