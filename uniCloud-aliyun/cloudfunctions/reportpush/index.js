...
exports.main = async (event, context) => {
	// event新增返回数据字段: result
	event.result = [];
	// 请求产品订单列表
	const db = uniCloud.databaseForJQL();
	const query = event.sids ? {
		sid: db.command.in(event.sids)
	} : {
		sid: db.command.exists(true)
	};
	const orderRes = await db.collection('health-product-order').where(query).field('uid, sid, name').get();
	console.log("ReportPush, matched orders: ", orderRes);
	const products = orderRes.data;
	if (products && products.length > 0) {
		const uids = products.map(item => {
			return item.uid;
		});
		console.log("ReportPush, matched uids: ", uids);
		const userRes = await db.collection('uni-id-users').where({
			_id: db.command.in(uids)
		}).field('token, wx_openid, nickname, products').get();
		console.log("ReportPush, matched users: ", userRes);
		const users = userRes.data;
		users.forEach(user => {
			if (user.products && user.products.length > 0) {
				const target = user.products[0];
				const matchedItems = products.filter(item => {
					return target.device && item.sid == target.device.sensorID;
				});
				console.log("ReportPush, user's products matched items: ", matchedItems);
				if (matchedItems && matchedItems.length > 0) {
					const product = matchedItems[0];
					product.token = user.token; // 请求设备报告需要token
					product.wx_openid = user.wx_openid; // 发送订阅消息需要openid
					product.username = user.nickname || ''; // 发送订阅消息需要username
					product.nickname = target.nickname; // 发送订阅消息需要nickname
				}
			}
		});
		...
		for (let pi = 0; pi < products.length; pi++) {
			...
			if (product.sid.length <= 0) {
				// 未绑定设备，跳过
				continue;
			}
			console.log("ReportPush, fetch report for: ", product);
			// 获取请求token
			let xAuthToken = undefined;
			if (product.token && product.token.length > 0) {
				for (let ti = product.token.length - 1; ti >= 0; ti--) {
					const token = product.token[ti];
					const payload = await uniIDIns.checkToken(token);
					if (payload.uid && payload.uid.length > 0 && payload.code == 0) {
						xAuthToken = token;
						break;
					}
				}
			...
			const response = await uniCloud.httpclient.request(
				'https://chat.aicareme.cn/api/wechat/mqtt/sleepreport/page', {
					...
					data: {
						pageNo: 1,
						pageSize: 10,
						sensorId: product.sid
					},
					contentType: 'json', // 指定以application/json发送data内的数据
					dataType: 'json', // 指定返回值为json格式，自动进行parse
					timeout: 20000
				});
			console.log("ReportPush, request report, response = ", response);
			...
			if (reportList && reportList.length > 0) {
				...
				if (String(targetReport.businessData.EndTime).slice(0, 10) == today) {
					...
					const healthLevel = getHealthLevel(deviceScore);
					const reportResult =
						`${healthLevel}：心率${averageRate(targetData) + '次/分'} 深睡${deepSleep(targetData)}`;
					console.log("ReportPush, reportResult: " + reportResult + ", deviceScore = " + deviceScore +
					...
					const res = await uniSubscribeMsg.sendSubscribeMessage({
						touser: product.wx_openid.mp,
						template_id: 'jVLxl20cBYoPJ6cbNOki8EHb5-j4NrSwRjt42OGgbeU',
						page: 'pages/device/device', // 小程序页面地址
						miniprogram_state: 'formal', // 跳转小程序类型：developer为开发版；trial为体验版；formal为正式版；默认为正式版
						lang: 'zh_CN',
						data: params
					});
					console.log("ReportPush, send subscribe message, response = ", res);
					event.result.push(res);
				}
			}
		}
	}
...
function getHealthLevel(score) {
	return score >= 70 ? '非常健康' : (score < 50 ? '轻度风险' : '中度风险');
}
...
function deepSleep(report) {
	const quality = report.MedicineSleepQuality;
	const deepSeconds = quality.DeepSleepRate * quality.TotalSleepTime;
	const hours = Number(deepSeconds / 3600).toFixed(1);
	return `${hours}小时`;
}
...
function parseReportHRs(report) {
	...
	console.log("parseReportHRs, sum = " + sum + ", validLength = " + validLength + ", max = " + max + ", min = " +
		min + ", rate = " + report.MedicineSleepQuality.HeartbeatRate);
	if (max > 0) {
...
function uatob(input) {
	...
	let output = '';
	if (str.length % 4 === 1) {
		console.log("ReportPush, uatob InvalidLengthError: str.length = " + str.length);
		return;
	}
