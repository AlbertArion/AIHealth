...
module.exports = async function(params = {}) {
	...
	const accessTokenMP = await getWeixinAccessToken.call(this)
	const res = await weixinApi.getPhoneNumber(accessTokenMP, params.phoneNumberCode);
	...
	if (type === 'register' && weixinPlatform !== 'mp') {
		...
		if (avatar) {
			...
			const cloudPath = `/upload/user/avatar/${openid.slice(-8) + Date.now()}-avatar.${extName}`
	...
	const postLoginResult = await postUnifiedLogin.call(this, {
		user,
		extraData: {
			...extraData,
