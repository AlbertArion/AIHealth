	...
	export default {
		...
		methods: {
			...
			initAsrListener: function() {
				if (asrRecorder.listener == 'followup') {
					return;
				}
				asrRecorder.listener = 'followup';
				// 开始识别
			...
			startCall: function() {
				console.log('ChattyInput, 智能随访, 开始通话');
				this.calling = true;
				this.$emit('last-message', (isFinished, message) => {
					let text = '';
					if (isFinished) {
						text = '您本轮的随访对话已结束，请问还有什么可以帮您？';
					} else if (message.user != 'home' || message._id == '000000000000000000000000') {
						text = this.fromCheckIn ?
			...
			endCall: function() {
				console.log('ChattyInput, 智能随访, 结束通话');
				this.audio?.stop();
				this.endRecord();
				this.calling = false;
				this.$emit('end-call');
